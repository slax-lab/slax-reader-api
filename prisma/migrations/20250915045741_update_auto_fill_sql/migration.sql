...
BEGIN
    IF TG_OP = 'INSERT' AND NEW.is_deleted = false THEN
        SELECT uuid INTO tag_uuid_val FROM sr_user_tag WHERE id = NEW.tag_id;
        UPDATE sr_user_bookmark
        SET metadata = jsonb_set(
            COALESCE(metadata, '{"tags": []}'::jsonb),
            '{tags}',
            COALESCE(metadata->'tags', '[]'::jsonb) || to_jsonb(tag_uuid_val)
        )
        WHERE user_id = NEW.user_id AND bookmark_id = NEW.bookmark_id;
        
    ELSIF TG_OP = 'DELETE' OR (TG_OP = 'UPDATE' AND NEW.is_deleted = true) THEN
        SELECT uuid INTO tag_uuid_val FROM sr_user_tag WHERE id = COALESCE(NEW.tag_id, OLD.tag_id);
        UPDATE sr_user_bookmark
        SET metadata = jsonb_set(
            COALESCE(metadata, '{"tags": []}'::jsonb),
            '{tags}',
            COALESCE(
                (SELECT jsonb_agg(elem)
                 ...
                 WHERE elem != to_jsonb(tag_uuid_val)),
                '[]'::jsonb
            )
        )
    ...
    ELSIF TG_OP = 'UPDATE' AND OLD.is_deleted = true AND NEW.is_deleted = false THEN
        ...
        SET metadata = jsonb_set(
            ...
            '{tags}',
            CASE 
                WHEN metadata->'tags' @> to_jsonb(ARRAY[tag_uuid_val])::jsonb 
                THEN metadata->'tags'
                ELSE COALESCE(metadata->'tags', '[]'::jsonb) || to_jsonb(tag_uuid_val)
            END
        )
        WHERE user_id = NEW.user_id AND bookmark_id = NEW.bookmark_id;
    END IF;
...
SET metadata = jsonb_build_object(
    ...
    'tags', COALESCE(
        (SELECT jsonb_agg(ut.uuid ORDER BY t.created_at DESC)
         FROM sr_user_bookmark_tag t
         JOIN sr_user_tag ut ON ut.id = t.tag_id
