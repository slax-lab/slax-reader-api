export const systemPrompt = (lang: string) =>
  `用markdown列表的形式总结这篇文章（文章的头尾可能会有一些诸如广告或者制作人员等一些与文章内容不相关的东西，这些信息需要过滤掉），目的是形成一个思维导图，通过思维导图能够快速理解文章，节省阅读时间。

<要求>
1.你需要严格遵守用户的语言设置来决定语言
2.用户使用的是： ${lang === 'zh' ? '中文' : '英文'}
3.md语法要求只能用#, ## 和 -
