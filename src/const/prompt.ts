...
- 你必须先完整回答用户的问题，在回答完问题后，你需要根据上下文生成3个与当前问题相关的问题（如果用户是在恶意攻击，则生成与文章主题相关的问题）。首先输出标签：“<relatedQuestionStart>”，然后再去进行相关问题生成，将最终结果多次调用\`relatedQuestion\`方法进行输出，除此之外不用说明任何其他东西。`
}
