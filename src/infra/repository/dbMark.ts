...
export class MarkRepo {
  ...
  async create(data: markPO): Promise<markPOWithId> {
    return await this.prismaPg().sr_mark_comment.create({
      data: {
  ...
  async list(userBmId: number) {
    return (
      await this.prismaPg().sr_mark_comment.findMany({
        where: {
  ...
  async get(id: number): Promise<markDetailPO | null> {
    const res = await this.prismaPg().sr_mark_comment.findFirst({ where: { id } })
    if (!res) return null
    return {
      id: res.id,
  ...
  async del(id: number) {
    return await this.prismaPg().sr_mark_comment.delete({ where: { id } })
  }

  async deleteByRootId(bookmarkId: number, rootId: number) {
    return await this.prismaPg().sr_mark_comment.deleteMany({ where: { bookmark_id: bookmarkId, root_id: rootId } })
  }

  async existsCommentMarkChild(bookmarkId: number, rootId: number) {
    return await this.prismaPg().sr_mark_comment.count({
      where: {
  ...
  async updateCommentMarkDeleted(id: number) {
    return await this.prismaPg().sr_mark_comment.update({ where: { id }, data: { is_deleted: true, updated_at: new Date() } })
  }

  async updateCommentRootId(id: number, rootId: number) {
    return await this.prismaPg().sr_mark_comment.update({ where: { id }, data: { root_id: rootId, updated_at: new Date() } })
  }

  async deleteByBookmarkId(bookmarkId: number) {
    return await this.prismaPg().sr_mark_comment.deleteMany({ where: { bookmark_id: bookmarkId } })
  }

  async listUserMark(userId: number, page: number, size: number) {
    return await this.prismaPg().sr_mark_comment.findMany({
